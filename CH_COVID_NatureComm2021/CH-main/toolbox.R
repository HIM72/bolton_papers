...
get_counts <- function(dat, grp1, grp2){
  grp1 <- enquo(grp1)
  grp2 <- enquo(grp2)
  tmp <- dat %>% group_by(!!grp1, !!grp2) %>% count() %>% ungroup()
  
  case <- tmp[tmp[, 1] == 1 & tmp[, 2] == 0, ]$n[1]
  control <- tmp[tmp[, 1] == 0 & tmp[, 2] == 0, ]$n[1]
  case_with_var <- tmp[tmp[, 1] == 1 & tmp[, 2] == 1, ]$n[1]
  control_with_var <- tmp[tmp[, 1] == 0 & tmp[, 2] == 1, ]$n[1]
  data.frame("case" = case, "control" = control, "case_with_var" = case_with_var, "control_with_var" = control_with_var)
}


do_glm_and_plot_or <- function(data = NULL, response = NULL, terms_for_formula = NULL,  terms_to_iterate = NULL, terms_to_plot = NULL, max_OR = NULL, plot_name = NULL, width = 10, height = 4){
  
  logit_gene_var = c()
  
  for (trm in terms_to_iterate) {
    cat("Analysing term :", trm, "\n")
    #set up formula
    glm_formula = as.formula(paste(
      response, 
      paste(terms_for_formula, get("trm"), sep = "+"), 
      sep = " ~ "
    ))
    #print(glm_formula)
    #do glm 
    logit = glm(
      ...
      family = "binomial")
    # get ORs
    logit_gene_var = rbind(logit_gene_var, 
                           cbind(
                             tidy(logit, exponentiate = T, conf.int = T, conf.level = .95) %>%
                               mutate(term_analyzed = trm),
                             get_counts(data, get(response), get(trm))
                             ) 
                           )
    
  }
  
  logit_gene_var <- logit_gene_var %>% 
    mutate(or_label = paste(round(estimate, 2), 
                            "\n[", 
                            round(conf.low, 2), "-", 
                            round(conf.high, 2), 
                            "]", sep = ""), 
           n = row_number(), 
           conf.high.arrow = ifelse(conf.high > max_OR, 1, 0), 
           conf.high.edited = ifelse(conf.high > max_OR, max_OR, conf.high)) 
  logit_to_return <- logit_gene_var
  if(is.null(terms_to_plot)){
    logit_gene_var <- logit_gene_var %>% filter(term %in% terms_to_iterate)
    logit_gene_var$term <- factor(logit_gene_var$term, levels = rev(terms_to_iterate))
  }else{
    logit_gene_var <- logit_gene_var %>% filter(term %in% terms_to_plot)
    logit_gene_var$term <- factor(logit_gene_var$term, levels = rev(terms_to_plot))
  }
  nrow = nrow(logit_gene_var)
  a <- ggplot(logit_gene_var, aes(x = 0, xend = max_OR, y = nrow+3, yend = nrow+3)) + 
    geom_point(aes(x = estimate, y = term)) + 
    geom_errorbarh(aes(xmin = conf.low, xmax = conf.high.edited, y = term), height = 0) + 
    geom_vline(xintercept = 1, color = "grey", linetype = "dashed") + 
    ylab("") + 
    xlab("OR") +
    scale_x_continuous(breaks = seq(0, max_OR, 1)) + 
    geom_text(aes(x = max_OR + 2, y = term, label = or_label), size = 3.5) + 
    geom_text(aes(x = max_OR + 4, y = term, label = round(p.value, 4)), size = 3.5) + 
    geom_text(aes(x = max_OR + 6, y = term, label = prettyNum(case, big.mark=",")), size = 3.5) + 
    geom_text(aes(x = max_OR + 8, y = term, label = prettyNum(control, big.mark=",")), size = 3.5) + 
    geom_text(aes(x = max_OR + 10, y = term, label = prettyNum(case_with_var, big.mark=",")), size = 3.5) + 
    geom_text(aes(x = max_OR + 12.5, y = term, label = prettyNum(control_with_var, big.mark=",")), size = 3.5) + 
    annotate("text", x = c(max_OR/2, max_OR + 2, max_OR + 4, max_OR + 6, max_OR + 8, max_OR + 10, max_OR + 12.5), y = nrow+1, label = c("Effect of CH\non COVID severity", "OR\n[95% CI]", "P", "Cases (N)", "Controls (N)", "Cases\nwith CH type (N)", "Controls\nwith CH type (N)"), hjust = 0.5, fontface = 2)  
  if(sum(logit_gene_var$conf.high.arrow) > 0){
    a <- a + geom_segment(data = logit_gene_var %>% filter(conf.high.arrow == 1), aes(x = estimate, xend = max_OR, y = term, yend = term), arrow = arrow(length = unit(0.3, "cm") ) )
  }
  print(a)
  ggsave(plot_name, plot = a, width = width, height = height)
  logit_to_return
}
